...
export default function HeadOffices() {
  ...
  async function onDelete(row) {
    if (!confirm(`"${row.name}" 본사를 삭제할까요?\n(해당 본사 가맹점/상품이 있으면 실패하도록 하는게 안전합니다)`)) return;
            ...
            {loading ? (
              ...
              items.map((row) => (
                <tr key={row.id} style={{ borderTop: "1px solid #eee" }}>
                  ...
                  <td style={td}>
                    <span style={{ padding: "6px 10px", background: "#f2f3f5", borderRadius: 10, fontWeight: 700 }}>
                      {row.code}
                    </span>
                  </td>
                  <td style={{ ...td, textAlign: "right" }}>
                    {/* ✅ 수정/삭제 */}
                    <button onClick={() => openEdit(row)} style={iconBtn}>✏️</button>
                    <button onClick={() => onDelete(row)} style={{ ...iconBtn, color: "#d11" }}>🗑️</button>
                  </td>
                </tr>
              ))
            )}
          ...
          <div style={modal}>
            ...
            <form onSubmit={onSubmit} style={{ marginTop: 16, display: "grid", gap: 12 }}>
              ...
              <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 6 }}>
                <button type="button" onClick={() => setOpen(false)} style={btnGhost}>취소</button>
